...
// src/command-history.tsx
var command_history_exports = {};
__export(command_history_exports, {
  default: () => Command
});
module.exports = __toCommonJS(command_history_exports);
...
var runesToStr = (runes) => runes.map((r) => String.fromCodePoint(r)).join("");
...
var isWhitespace = (rune) => whitespaceRunes.has(rune);
var whitespacesAtStart = (runes) => {
  let whitespaces = 0;
...
var prefixMatch = (caseSensitive, normalize, forward, text, pattern, withPos, slab2) => {
  if (pattern.length === 0) {
    return [{ start: 0, end: 0, score: 0 }, null];
  }
  let trimmedLen = 0;
...
var suffixMatch = (caseSensitive, normalize, forward, text, pattern, withPos, slab2) => {
  const lenRunes = text.length;
...
var equalMatch = (caseSensitive, normalize, forward, text, pattern, withPos, slab2) => {
  const lenPattern = pattern.length;
  if (lenPattern === 0) {
    return [{ start: -1, end: -1, score: 0 }, null];
  }
...
var HISTORY_PATH = `${(0, import_os.homedir)()}/.zsh_history`;
var MAX_HISTORY_ITEMS = 1e4;
var MAX_VISIBLE_ITEMS = 300;
...
async function loadZshHistory() {
  const raw = await (0, import_promises.readFile)(HISTORY_PATH, "utf8");
...
async function fzfFilter(items, query) {
  if (!query.trim()) {
...
function Command() {
  ...
  return /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
    import_api.List,
    {
      ...
      filtering: false,
      throttle: true,
      children: errorText && results.length === 0 ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_api.List.EmptyView, { title: "Fehler", description: errorText, icon: import_api.Icon.ExclamationMark }) : results.length === 0 && !isLoading ? /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
        import_api.List.EmptyView,
        {
          title: searchText ? "Keine Treffer" : "History geladen",
          description: searchText ? `Keine Befehle f\xFCr "${searchText}" gefunden.` : "Fang an zu tippen, um zu suchen.",
          icon: searchText ? import_api.Icon.MagnifyingGlass : import_api.Icon.Terminal
        }
      ) : /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_api.List.Section, { title: "zsh history", subtitle, children: results.map((command, index) => /* @__PURE__ */ (0, import_jsx_runtime.jsx)(
        import_api.List.Item,
        {
          ...
          actions: /* @__PURE__ */ (0, import_jsx_runtime.jsxs)(import_api.ActionPanel, { children: [
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_api.Action.Paste, { title: "In Aktive App Einfuegen", content: command }),
            /* @__PURE__ */ (0, import_jsx_runtime.jsx)(import_api.Action.CopyToClipboard, { title: "In Zwischenablage Kopieren", content: command })
          ] })
        },
        `${command}-${index}`
      )) })
